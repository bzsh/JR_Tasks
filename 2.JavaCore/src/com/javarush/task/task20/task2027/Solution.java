...
same - (1, 1) - (4, 1)
    ...
    public static List<Word> detectAllWords(int[][] crossword, String... words) {
        ...
        for (String word : words) { // перебираем слова из words (String... words - массив стрингов из слов переданных в метод из main)
            chars = word.toCharArray();
            for (int i = 0; i < crossword.length; i++) { // перебираем массив crossword
                for (int j = 0; j < crossword[i].length; j++) {
                    if (crossword[i][j] == chars[0]) {             // как толко первый символ найден, сразу ищем следующие из chars
                        HashMap<Integer, StringBuilder> temp = new HashMap<>(); // создали коллекцию, или пересоздали в случае нахождения еще одного первого(стартового)символа
                        for (int k = 1; k < chars.length; k++) {
                            for (int c = 0; c < 8; c++) {
                                switch (c) {                        // проверяем по направлениям от найденного первого символа
                                    case (0):
                                        tempI = i - k;
                                        tempJ = j;
                                        break;                     // ↑
                                ...
                                if (tempI >= 0 && tempI < crossword.length) {      //проверка на не выход за пределы массива
                                    if (tempJ >= 0 && tempJ < crossword[i].length) {

                                        if (k == 1) { // на совпадении второй буквы создаем StringBuilder и забрасываем его в коллекцию
                                            ...
                                            temp.get(c).append((char) crossword[tempI][tempJ]); // если нашли 3ю букву(любую букву)просто дополняем обьект прямо в коллекции
                                        }

                                        if (k == chars.length - 1) { // если последний символ то добавляем его кооринаты
                                            temp.get(c).append(" ");
